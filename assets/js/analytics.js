...
class NostrAnalytics {
  ...
  getClientStats(limit = 20) {
    const clients = Array.from(this.clientStats.entries())
      .map(([name, stats]) => {
        // Handle kinds - if it's already an object, use it directly; if it's a Map or entries array, convert it
        let kindsList = stats.kinds;
        if (stats.kinds && typeof stats.kinds === 'object' && !Array.isArray(stats.kinds)) {
          // Already an object, use it as-is
          kindsList = stats.kinds;
        } else if (Array.isArray(stats.kinds) || (stats.kinds instanceof Map)) {
          // If it's an array of entries or a Map, convert to object
          kindsList = Object.fromEntries(stats.kinds);
        } else {
  ...
  // Load client stats from backend API data
  loadClientStatsFromBackend(backendClients) {
    if (!backendClients || !Array.isArray(backendClients)) return;
    
    // Clear existing client stats
    this.clientStats.clear();
    
    // Get top 3 event kinds for display (from global eventKindCounts if available)
    let topKinds = {};
    if (typeof eventKindCounts !== 'undefined' && eventKindCounts) {
      topKinds = Object.entries(eventKindCounts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
        .reduce((acc, [kind]) => {
          acc[kind] = 1;
    ...
    backendClients.forEach(client => {
      if (client.name && client.count) {
        this.clientStats.set(client.name, {
          count: client.count,
          lastSeen: Math.floor(Date.now() / 1000), // Use current time as approximation
          kinds: topKinds, // Use top kinds from global distribution
          firstSeen: Math.floor(Date.now() / 1000) - (24 * 60 * 60) // Approximate 24h ago
        });
      }
    });
    
    console.log(`Loaded ${this.clientStats.size} clients from backend`);
    
    // Notify listeners that client data has been updated
    this.notifyListeners('clientsLoaded', { count: this.clientStats.size });
  }
